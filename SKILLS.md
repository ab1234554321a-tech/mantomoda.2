...
2. **اسکیل‌ها فقط دستورالعمل نیستند — اجرا شده‌اند و کد را تغییر داده‌اند.** نتیجه در بخش ۸ ثبت شده است؛ اسکریپت‌های همراهشان (`scripts/*.sh`, `*.py`) قبل از اجرا مرور شوند. در این مجموعه هیچ الگوی `curl | bash` در ست انتخابی وجود ندارد (تنها مورد پرچم‌دار، اسکیل `automatic-stateful-prompt-improver` بود که نصب نشد).
...
## ۸. اسکیل‌ها در عمل — دو باگ واقعی که پیدا کردند (ADR-023)

تا قبل از این مرحله، اسکیل‌ها فقط «نصب شده» بودند. حالا اسکریپت‌های خودشان روی `src/` اجرا می‌شوند و
خروجی‌شان روی پروژه اثر گذاشته است. با یک دستور همه‌شان دوباره اجرا می‌شوند:

```bash
npm run skills-audit        # یا: bash scripts/skills-audit.sh
```

گزارش با تاریخ در `reports/skills-audit-<تاریخ>.md` ساخته می‌شود و شامل: اسکن OWASP، اسکن سکرت،
پوشش مستندات و ADR، تست‌ها، دسترس‌پذیری، `npm audit` و `preflight`.

### یافته ۱ — XSS ذخیره‌شده در پنل مدیر (بحرانی، واقعی)

`security-auditor` روی استفاده از `innerHTML` در کلاینت هشدار داد. بررسی دستی نشان داد فیلدهایی که
**خود مشتری** پر می‌کند (نام شرکت، آدرس، تلفن، شهر، کد اقتصادی در فرم درخواست عمده‌فروشی، و نام/ایمیل
هنگام ثبت‌نام) بدون escape داخل **پنل مدیر** رندر می‌شدند. یعنی یک مشتری می‌توانست با نام شرکت یک کد
ذخیره کند و لحظه‌ای که صاحب فروشگاه فهرست درخواست‌های عمده را باز می‌کرد، آن کد در مرورگر او اجرا
می‌شد — با دسترسی به نشست مدیر و امکان خواندن نام، تلفن و آدرس همه مشتریان.

**اصلاح:** یک تابع escape واحد (`escapeHtml`/`escapeAttr`) روی هر ۳۶ نقطه‌ای که متن نوشته‌شده توسط
انسان رندر می‌شود؛ به‌علاوه استفاده از `textContent`/`value` در جاهایی که ممکن بود. **قفل:** مجموعه
تست سطح ۹ (`tests/escaping.test.js`) با تست جهش (mutation) تأیید شد: برگرداندن باگ ⇒ تست قرمز.
سند تصمیم: `ADR-022`.

### یافته ۲ — برخورد شماره سفارش (واقعی)

`owasp-check.py` روی `Math.random()` هشدار A02 داد. شماره سفارش از `1000 + rand*9000` ساخته می‌شد،
یعنی فقط ۹۰۰۰ مقدار ممکن؛ با حدود ۱۱۰ سفارش در سال، احتمال برخورد ~۵۰٪ است. و چون **فاکتور با شماره
سفارش** پیدا می‌شود، یعنی مشتری می‌توانست فاکتور مشتری دیگری را ببیند.

**اصلاح:** شماره سفارش از یک شمارنده یکنوا (`orderSequence`) که در snapshot ذخیره می‌شود، با شروع
تصادفی و حلقه ضدبرخورد؛ شناسه محصول/کوپن/لاگ و `refId` پرداخت هم به `crypto` منتقل شدند.
**قفل:** تست یکتایی ۳۰۰ سفارش + یکتایی جست‌وجوی فاکتور در مجموعه commerce.

### قاعده خواندن گزارش اسکیل‌ها

اسکنر **کاندید** تولید می‌کند، نه حکم. در اولین اجرا ۷ مورد «CRITICAL» تزریق SQL گزارش شد در حالی که
پروژه هیچ SQL ندارد (ذخیره‌سازی JSON snapshot است، ADR-010) و از ۳۹ هشدار `innerHTML` فقط یک زنجیره
واقعاً قابل بهره‌برداری بود. پس: اجرا کن، بخوان، دستی راستی‌آزمایی کن، آنچه واقعی است را درست کن و
آنچه کاذب است را ثبت کن. هر یافته‌ی واقعی به یک **تست واقعی** ارتقا پیدا می‌کند و فقط تست‌ها اجازه
توقف انتشار دارند؛ به همین دلیل این بازرسی «مشورتی» است و CI مرجع نهایی می‌ماند.
